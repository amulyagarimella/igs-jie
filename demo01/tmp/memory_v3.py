...
# compute chr-chr distance using all pairs distances between fibers
# PCA cell similarity 
...
for chr_file in available_chr:
    ...
    cell_chr_fiber.to_csv("cell_chr_fiber_full_chr123.csv")
    print("completed")"""
...
# start with components = none
ppca = PPCA()
ppca.fit(data=chr_vs_chr,d=2,verbose=True)
# TODO graph
...
# if you dropna for PCA, need to save which cell indices ur keeping
same_fov = []
diff_fov = [] 
for i in range(len(cells)):
    for j in range(i+1,len(cells)):
        try:
            sim = math.dist(chr_vs_chr_transformed[i], chr_vs_chr_transformed[j])
            print(sim)
        except:
        ...
        if np.isnan(sim):
            continue
        cell_v_cell[i][j][0] = sim
        cell_v_cell[j][i][0] = sim
...
plt.savefig("chr_all_memviz_ppca.png")
plt.savefig("chr_all_memviz_ppca.svg")
plt.show()

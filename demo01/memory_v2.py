...
# this version flattened chr v chr matrix
...
for chr_file in available_chr[0:4]:
    print(chr_file)
    ...
    for i in range(len(chr_fibers)):
        ...
        for c in range(len(all_cell_fibers)):
            ...
            # TODO inflection pt ~ save all pts in dict?
            try:
...
for c in cells:
    ...
    for i in range(len(chrs)):
        ...
        for j in range(i+1,len(chrs)):
            ...
            #print(dists)
            cvc[i*len(chrs)+j-1] = np.nanmean(dists)
    chr_vs_chr[cells.index(c),:] = cvc
    #print(cvc)
    #print(chr_vs_chr)
print(chr_vs_chr)
print(chr_vs_chr[:,~np.isnan(chr_vs_chr).any(axis=0)])
